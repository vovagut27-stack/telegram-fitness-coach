...
import { countCompletedThisWeek, getRecentWorkouts, getWorkoutByDate, saveWorkoutPlan, } from "../database/workouts-repo.js";
import { AIWorkoutService } from "./ai-service.js";
const aiService = new AIWorkoutService();
export async function ensureDefaultUser(telegramId) {
...
function muscleRotationFromHistory(history) {
    const cycle = [
...
export async function getOrCreateTodayWorkout(telegramId) {
...
function adjustDifficulty(currentLevel, completedThisWeek) {
...
export async function canGenerateWorkout(telegramId) {
    const user = await getUser(telegramId);
    if (user?.isPremium) {
        return true;
    }
